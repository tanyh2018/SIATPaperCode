...
def GenCircularCoordAndRadius(base_r, base_x, base_y):
    ...
    # base_x = random.randint(center_coord_offset[0], center_coord_offset[1]) + (img_size // 2)
    # base_y = random.randint(center_coord_offset[0], center_coord_offset[1]) + (img_size // 2)  # 随机生成基圆中心坐标点
...
def DrawImage(circular_list):
    ...
    :return:
    """
    label_I2 = np.zeros([img_size, img_size], dtype=np.float32)
    # index = 0
    # 画I2
    for circular in circular_list[1:]:
        ...
        # I2_density = I2_density_range[index]
        # index = index + 1
...
if __name__ == '__main__':
    ####圆形测试
    # circular_list = GenCircularCoordAndRadius()
    # print(temp)
    # img = np.zeros((512, 512), np.float32)  # 生成一个空灰度图像
    # for i in range(len(temp)):
    #     point_color = (0.258)  # grey
    #     cv2.circle(img, (temp[i][0], temp[i][1]), temp[i][2], point_color, -1)
    #
    # img = np.asarray(img)
    # img.astype(np.float32).tofile('./test.raw')
    ...
    for i in range(50):
        circular_list = GenCircularCoordAndRadius()
        label_water, label_cacl2, label_I2 = DrawImage(circular_list)
        label_water.astype(np.float32).tofile('./Training_data/Basis/label_water_{}.raw'.format(i))
        label_cacl2.astype(np.float32).tofile('./Training_data/Basis/label_cacl2_{}.raw'.format(i))
