...
def genbasis_imgs(img_path):
    ...
    if (np.sum(bone_mask == 1) / np.sum(Circ_mask == 1)) <= 0.4 and (
        ...
        rmask = mask_result_generate()
        Basis_bone = np.where(rmask, Basis_bone, 0)
        print(Basis_bone.shape)
        Basis_bone.astype(np.float32).tofile(sys.argv[2] + '{}_label_twin.raw'.format(name))

def mask_result_generate():
    A = np.zeros((img_size, img_size))

    # 中心坐标
    center = (int(img_size/2.0), int(img_size/2.0))
    ...
    # 将高斯分布乘以半径范围，使半径在不同方向上缓慢变化
    min_radius = zero_range[0]
    max_radius = zero_range[1]
    radius = min_radius + (max_radius - min_radius) * gaussian
