...
filePath = 'D:/xianjinyuan/工作内容/Jiecheng/NanoCT/Simulation/NanoCT_Sim/SimCode/withoutZP/data/split_test4/'
filePathout=[filePath,'CTresult/']
filePath=fullfile(filePath);
dirOutput=dir(fullfile(filePath,'*sd*.raw'));  
filename = {dirOutput.name};
%filename={'period_8_sep_2.24_sigma_0.0_absor_pixel_size_48.0_m1_sd_0.01.raw'};
...
for ii = 1:length(filename)
    ...
    %systerm information
    d1=1.382;d2=0.104;d3=0.181; %unit m
...
%     img(1:50,:,:)=img(1:50,:,:)*(-1.0);
    ...
    % A = Proj(:,:,100); 
    %% Parameter settings
    nx = 200;ny = nx; % DPCT image size
    dx = single(24E-6);dy= dx;  %pixel size: dx*dy; Default Unit: m dy_det/dx=nx/px
    dy_det=single(48E-6); %detector pixel size, Default Unit: m
    ...
    imshow(DPCT,[]);
%     figure,plot(cumulative_sum); 
%     figure,plot(cumulative_sum1); 
