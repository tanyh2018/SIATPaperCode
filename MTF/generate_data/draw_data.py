...
def main():
      # delta_s_effect()   # graphite
      detector_pixel_effect()
      #material_effect()
      # sample_size_effect()
      ...
      # #dir_pre = ['LiF','water','teflon','mylar','polyimide']
      #dir_path = '/data4/tanyuhang/data/talbot_result/graphite_sd/20221212_graphite/'
      # dir_pre = ['graphite']
      # #dir_pre = ['bonecortical','aluminum','graphite','mgo','PMMA']
      # dir_pre = ['water','graphite','mylar','PMMA']
      # dir_pre = ['20221025_aluminum','20221025_aluminum_sample_size_400','20221025_aluminum_sample_size_500','20221025_sample_size_400','20221025_sample_size_500']
      # dir_path = '/data2/tanyuhang/data/talbot_result/20221212_'
      # dir_pre = ['graphite']
      # for j in range(5):
            #dir_paths = dir_path + dir_pre[j]+'/'

            # file_name_list = os.listdir(dir_path_prex)
            # for j in range(len(file_name_list)):
            #       if 'obj_split' in file_name_list[j]:
            #             Test = True


            # # run batch jobs
            # # if args[0] in dir_path_prex:
            # if Test and 'period_5.13' not in dir_path_prex:
            #       print(dir_path_prex)
      ...
      for j in range(len(names)):
            ...
            for i in range(len(dir_path_prexs)):
                  ...
                  for k in range(0,1):
                        ...
                        draw_linesource_figure(dir_path_prex,pixel_size,colormodel,sigma,sigma_model) 

def material_effect():
      names = ['20240522_mylar/','20240522_graphite/','20240522_PMMA/','20240522_water/']
      mnames = ['mylar','graphite','PMMA','water']
      ...
      for j in range(len(names)):
            ...
            for i in range(len(dir_path_prexs)):
                  ...
                  for k in range(0,1):
                        pixel_size=48
                        sigma = (1.45)*48/pixel_size
                        draw_linesource_figure(dir_path_prex,pixel_size,colormodel,sigma,sigma_model,mnames[j]) 

def detector_pixel_effect():
      # point source is sigma=1.55*0.01 for 48um pixel size
      #line source is sigma=1.55 for 48um pixel size
      gc.set_threshold(500,5,5)
      dir_path = '/data2/tanyuhang/data/talbot_result/20221212_graphite/'
      dir_paths = dir_path
      dir_path_prexs = os.listdir(dir_paths)
...
def delta_s_effect():
      ...
      for i in range(len(dir_path_prexs)):
            dir_path_prex = dir_paths + dir_path_prexs[i]+'/'
            period = float((dir_path_prexs[i]).split('_')[1])
            draw_linesource_figure(dir_path_prex,pixel_size,colormodel,sigma_model) 


def draw_linesource_figure(dir_path_prex,pixel_size,colormodel,sigma,sigma_model,material='graphite'):
      '''  Get the intensity and phase result'''
      obj_data, bkg_data = linesource_data_file(dir_path_prex,colormodel,sigma_model) 
      i=5
      for j in range(1):
            ...
            obj_data=obj_data.reshape((step_n,int(total_number/1000)))
            bkg_data=bkg_data.reshape((step_n,int(total_number/1000)))
            FTout_obj=get_gauss_filter(obj_data,pixel_number,sigma)
            FTout_bkg=get_gauss_filter(bkg_data,pixel_number,sigma)
            ...
            save_raw_data(phi_final,dir_path_prex + dir_path_prex.split('/')[-2] + '_sigma_' + str(round(sigma,2)) + '_phi_'+'pixel_size_'+str(pixel_size)+'_'+materiral+'_'+colormodel+sigma_model+'.raw') #single_energy sprectrum
            save_raw_data(absor_final,dir_path_prex + dir_path_prex.split('/')[-2] + '_sigma_' + str(round(sigma,2)) + '_absor_'+'pixel_size_'+str(pixel_size)+'_'+materiral+'_'+colormodel+sigma_model+'.raw')
            save_imshow(phi_final,absor_final,dir_path_prex +'sigma_'+str(sigma) +'_pixel_'+str(pixel_size) + '_imshowfft.png')  
            save_raw_data(phi_final,'./detector_pixel/' + dir_path_prex.split('/')[-2] + '_sigma_' + str(round(sigma,2)) + '_phi_'+'pixel_size_'+str(pixel_size)+'_'+materiral+'_'+colormodel+sigma_model+'_'+material+'.raw') #single_energy sprectrum
            save_raw_data(absor_final,'./detector_pixel/' + dir_path_prex.split('/')[-2] + '_sigma_' + str(round(sigma,2)) + '_absor_'+'pixel_size_'+str(pixel_size)+'_'+materiral+'_'+colormodel+sigma_model+'_'+material+'.raw')
            ...
            # # absor_final_2 = -np.log(abs_img/abs_zimg_bkg)
...
def save_reshape_result(input_list,dir_path_prex,model, colormodel):
      #second save
      FDetT1s = []
      pixel_number = 1000
      for i in range(len(input_list)):
            FDetT1s.append(gauss_filter_reshape(input_list[i],int(len(input_list[i])/1000),0))
      data = np.array(FDetT1s)
      if model == 'signal':
            file_name_real = dir_path_prex +colormodel + '_obj_split.raw'
      elif model == 'bkg':
            file_name_real = dir_path_prex +colormodel + '_bkg_split.raw'
...
def linesource_data_file(input_path,colormodel,signal_model):
      ''' Read datas '''
      file_name_list = os.listdir(input_path)
      dir_path_obj_real = [ "".join([input_path,'/',file_name_list[j]]) for j in range(len(file_name_list)) if ('obj_split' in file_name_list[j] and 'raw' in file_name_list[j] and colormodel in file_name_list[j] and signal_model in file_name_list[j])]
...
def gauss_filter_reshape(data,pixel_number,gsigma):
      data = data[0:int(pixel_number*int(len(data)/pixel_number))]
