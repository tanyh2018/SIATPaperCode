...
%% Phase information
path_name ='.\data\';
file_name_I ='仿真聚苯乙烯-PMMA-水-phase-fanbeamtomo-600-parall.raw';
fid_2 = fopen([path_name file_name_I], 'r');   %存为raw
sino_1=fread(fid_2, [600 720],'float32','l');

%% Absor information
file_name_I ='仿真聚苯乙烯-PMMA-水-absor-fanbeamtomo-600-parall.raw';
fid_3 = fopen([path_name file_name_I], 'r');   %存为raw
sino_2=fread(fid_3, [600 720],'float32','l');
...
Nimg2 = size(phase);
% prior = rand([Nimg2(1),Nimg2(2)])*0.01;
prior = zeros(Nimg2);
figure;
imshow(phase, [])
title('Phase projection');
% phase(abs(phase)<1e-12)=0;
    ...
    for k=1:Nimg2(2)
        for j =1:Nimg2(1)
            if j <= N_shift1
                prior(j,k)= 0;
            elseif j <= 2*N_shift1
                prior(j,k)= -phase(j-N_shift1,k)/sr2;
            else
                prior(j,k)= (sr2*prior(j-2*N_shift1,k)-phase(j-N_shift1,k))/sr2;
            end
        end
    end
else
...
%                 prior(j,k)= 0;
%             elseif j<=2*N_shift1+2
%                 prior(j,k)= - phase(j-N_shift1-1,k)/sr1 ;
%             else
%                 prior(j,k) = (sr1*prior(j-2*N_shift1-2,k) - phase(j-N_shift1-1,k))/sr1;
%             end
%         end
%     end
% end
...
beta = 0.001; dt = 1; % paper: 500, 0.05, 1e-5 80
