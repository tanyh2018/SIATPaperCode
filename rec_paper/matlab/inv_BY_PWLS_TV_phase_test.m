
%% Get phase result when spliting distance is integer pixel   2023-4-23
%% Add non-integer pixel method   2023-4-23
clearvars;
clc;
%% 没有偏移没有噪声的相位原始投影图
path_name ='.\data\';
file_name_I ='仿真聚苯乙烯-PMMA-水-absor-fanbeamtomo-6000-tmp.raw';
fid_2 = fopen([path_name file_name_I], 'r');   %存为raw
sino_1=fread(fid_2, [6000 360],'float32','l');
figure;
imshow(sino_1, [])
title('Phase projection1');
%% 没有偏移没有噪声的吸收原始投影图
file_name_I ='仿真聚苯乙烯-PMMA-水-phase-fanbeamtomo-6000-tmp.raw';
fid_3 = fopen([path_name file_name_I], 'r');   %存为raw
sino_2=fread(fid_3, [6000 360],'float32','l');

%modeln = "No_noise";
modeln = "Noise";
model = "ACT";

%% 构造变换矩阵A  AX=Y
date = datestr(datetime, 'yyyy.mm.dd.HH.MM.ss');
for k=25
N_shift = k;
amp=10;
N_img = size(sino_1, 1)/amp;
A_1 = zeros(N_img*amp, N_img*amp);
A_2 = zeros(N_img*amp, N_img*amp);

A_1(N_shift*amp+1:end,1:end-N_shift*amp) = eye((N_img-N_shift)*amp,(N_img-N_shift)*amp);
A_2(1:end-N_shift*amp, N_shift*amp+1:end) = -eye((N_img-N_shift)*amp,(N_img-N_shift)*amp);

A_sum = A_1+A_2;

if amp > 1
    A_sum = reshape(A_sum,amp,amp,N_img,N_img);
    A_sum=mean(A_sum,1);
    A_sum=reshape(A_sum,N_img,amp,N_img);
    A_sum=mean(A_sum,2);
    A_sum=reshape(A_sum,N_img,N_img)*amp;
end 

for i=1:N_img
    for j=1:N_img
        if i==j
            A_sum(i,j)=A_sum(i,j) + 1e-12;
        end
    end
end

%% 对相位投影图进行偏移

Nimg2 = size(sino_1);
Nimg1 = Nimg2
sino_e=sino_1;
e=zeros(Nimg2);
c=zeros(Nimg2);
e(1:Nimg2(1)-N_shift*amp,:)=sino_e(N_shift*amp+1:Nimg2(1),:);
c(N_shift*amp+1:Nimg2(1),:)=sino_e(1:Nimg2(1)-N_shift*amp,:);
ori_phase=c-e;             %偏移后的 exp（-ul）
%plot and save
% figure;
% imshow(ori_phase, [])
% title('Phase projection1');

%% 对吸收投影图进行偏移
sino_e=exp(-sino_2);
e=ones(Nimg2);
c=ones(Nimg2);
e(1:Nimg2(1)-N_shift*amp,:)=sino_e(N_shift*amp+1:Nimg2(1),:);
c(N_shift*amp+1:Nimg2(1),:)=sino_e(1:Nimg2(1)-N_shift*amp,:);
ori_e=zeros(Nimg2);
ori_e=(e+c)/2;             %偏移后的 exp（-ul）

%% 对偏移图加泊松噪声
I_0=50000;
M=8;
I_1=ones(M,Nimg2(1),Nimg2(2));
II = I_0*ori_e;
for i=1:M
    for j =1:Nimg2(1)
        for k=1:Nimg2(2)
            I_1(i,j,k)=I_0*ori_e(j,k)*(1+0.7*cos(2*pi*i/M + ori_phase(j,k)));     %偏移后光强投影图I_0*exp（-ul）
        end
    end
end
ori_pois=zeros(Nimg2);
if strcmpi(modeln,'No_noise')
    ori_pois=I_1;   
else
    ori_pois=poissrnd(I_1);        
end

img_shifted=ori_pois/I_0;     %加泊松噪声后线积分图 exp（-ul）
phase = zeros(Nimg2(1),Nimg2(2));
I1s=zeros(Nimg2);
I1c=zeros(Nimg2);

for i=1:M
    for j =1:Nimg2(1)
        for k=1:Nimg2(2)
            I1s(j,k) = I1s(j,k) + img_shifted(i,j,k)*sin(2*pi*i/M);
            I1c(j,k) = I1c(j,k) + img_shifted(i,j,k)*cos(2*pi*i/M);
        end
    end
end

for j =1:Nimg2(1)
    for k=1:Nimg2(2)
        phase(j,k)= atan(-I1s(j,k)/I1c(j,k));
    end
end
if amp > 1
    phase = reshape(phase,amp,Nimg1(1)/amp,Nimg1(2));
    phase = mean(phase,1);
end
phase = reshape(phase,Nimg1(1)/amp,Nimg1(2));

Nimg2 = size(phase);
%Solving linear equations 
prior = zeros(Nimg2(1),Nimg2(2));

figure;
imshow(phase, [])
title('Phase projection2');

% %% Detail codes to achieve inverse
sr1 = abs(fix(N_shift)-N_shift);
sr2 = 1-abs(fix(N_shift)-N_shift);
N_shift1 = fix(N_shift);

% if sr1==0
%     for k=1:Nimg2(2)
%         for j =1:Nimg2(1)
%             if j <= N_shift1
%                 prior(j,k)= 0;
%             elseif j <= 2*N_shift1
%                 prior(j,k)= -phase(j-N_shift1,k);
%             else
%                 prior(j,k)= prior(j-2*N_shift1,k)-phase(j-N_shift1,k);
%             end
%         end
%     end
% else
%     for k=1:Nimg2(2)
%         for j =1:Nimg2(1)
%             if j <= N_shift1+1
%                 prior(j,k)= 0;
%             elseif j<=2*N_shift1+1
%                 prior(j,k)= (- sr2*prior(j-1,k)- phase(j-N_shift1-1,k))/sr1 ;
%             elseif j<=2*N_shift1+2
%                 prior(j,k)= (sr2*prior(j-2*N_shift1-1,k) - sr2*prior(j-1,k)- phase(j-N_shift1-1,k))/sr1;
%             else
%                 prior(j,k) = (sr1*prior(j-2*N_shift1-2,k) + sr2*prior(j-2*N_shift1-1,k)- sr2*prior(j-1,k) - phase(j-N_shift1-1,k))/sr1;
%             end
%         end
%     end
% end
prior=inv(A_sum)*phase;
figure;
imshow(prior, [])
title('Phase projection3');
%% 求解X
N = Nimg2(1);
Nx = Nimg2(2);
Ny = Nimg2(1);
iter = 80; % iter = 10 when di = 1; iter = 40 for di = 481; iter = 45 for di = 526; 迭代次数
beta = 0.01; dt = 1; % paper: 500, 0.05, 1e-5 80
% nx = N; ny =N;
a=10.^(-10);% a  - data small enough
tau=1e-5;
tic
A=A_sum;

%% 2D reconstrution
for j = 1:Nx
      b= phase(:,j); 
      I= prior(:,j);       
%% MI
        theta=eye(Ny);
        for i=1:Ny
            pi=mean(ori_pois(:,i,j));
            theta(i,i) = 2./(pi*M);
        end
     
        for i=1:iter  %% do iterations
            At = A*I(:);
            g = A'*(At-b);
%             g=A'*inv(theta)*(A*I(:)-b); 
%             tau=g'*g/((A*g)'*(inv(theta)*A*g));
            I_f=0-I;
            I_b=I;
            I_u=I-I([1 1:N-1]);
            I_d=I([2:N N])-I;
            I_x_1=0;
            I_y_1=I([2:N N]);
            den1=(a+I_u.^2).^(0.5);
            den3=(a+I_d.^2).^(0.5);
            v=(I_u)./den1-I_d./den3;
            norm=(sum(sum(v.^2))).^(0.5);
            v=v./norm;
            u=tau*g+beta*dt*v;
            I=I-u;
        end
        x_TV(:,j) = I;

    end
toc


% % 投影重建
DPCT = recfbp(x_TV,model,Nimg2);
DPCT_linear = recfbp(prior,model,Nimg2);
DPCT_phase = recfbp(phase,"DPC-CT",Nimg2)/N_shift;
figure;
subplot(2,2,1);
imshow(DPCT_linear,[]);
title('Linear equations rec');
subplot(2,2,2);
imshow(DPCT,[]);
title('Iterative rec');
subplot(2,2,3);
imshow(DPCT_phase,[]);
title('Direct rec');
% imshow(DPCT,[]);
% title('Iterative rec');
date = datestr(datetime, 'yyyy.mm.dd.HH.MM.ss');
filePathout="result/" +char(date)+"/";
if not(isfolder(filePathout))
    mkdir(filePathout)
end
% 
out_file = filePathout  + "I0_" + num2str(I_0) + "_beta_" + num2str(beta) + "_iter_" + num2str(iter)  + "_Nshift_"   + num2str(N_shift) + "_B_tan_OBP+PWLS-TV.raw";
fid = fopen([out_file], 'w');
fwrite(fid, DPCT, 'float32');
fclose(fid);

out_file = filePathout  + "I0_" + num2str(I_0) + "_beta_" + num2str(beta) + "_iter_" + num2str(iter)  + "_Nshift_"   + num2str(N_shift) + "_B_tan_OBP.raw";
fid = fopen([out_file], 'w');
fwrite(fid, DPCT_linear, 'float32');
fclose(fid);

out_file = filePathout  + "I0_" + num2str(I_0) + "_beta_" + num2str(beta) + "_iter_" + num2str(iter)  + "_Nshift_"   + num2str(N_shift) +"_B_tan_FBP.raw";
fid = fopen([out_file], 'w');
fwrite(fid, DPCT_phase, 'float32');
fclose(fid);


end