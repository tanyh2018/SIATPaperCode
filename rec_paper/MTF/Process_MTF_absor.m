...
%% ===== Read in files =====
pixel_size = 0.025E-4; % pixel dimension, unit: mm
row = 2048;
col = 2048;
%%% catphan
...
c_row = 1300; c_col =1300; % # of rows and cols in the cropped image
max_cluster = 11;
c_pix = 50;
...
%fileName = {'I0_1000_beta_0.02_iter_80_Nshift_1_B_tan_OBP+PWLS-TV'};
%fileName = {'I0_500_beta_0.1_iter_80_Nshift_1_B_tan_OBP'};
fileName = {'phase_MTF_image_delta'};
...
for ii = 1:length(fileName)
    ...
    fclose(fid);
    bkgIn = img(centerRow-c_row/2+1:centerRow+c_row/2, ce2nterCol-c_col/2+1:centerCol+c_col/2);
...
%%
    ...
    fid = fopen([filePathout, fileName{ii}, 'MTF_fit.txt'],'wt');
    m = size(xindex1);
    for i = 1:1:m
        fprintf(fid, '%g,%g \n', xindex1(i), MTF_f(i));
    end
    
    fid = fopen([filePathout, fileName{ii}, 'MTF.txt'],'wt');
    m = size(xindex1);
    for i = 1:1:m
        fprintf(fid, '%g,%g \n', xindex1(i), MTF(i));
    end
