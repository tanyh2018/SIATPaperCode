...
filePath = 'F:/Talbot_MTF/code_data/pv1_20221219/code_version/version_2022_12_19/python/NanoCTSim/data/split_test2/'
filePathout=[filePath,'CTresult/']
filePath=fullfile(filePath);
dirOutput=dir(fullfile(filePath,'*250*.raw'));  
...
for ii = 1:length(filename)
    ...
    %systerm information
    d1=1.382e10;d2=0.104;d3=0.181; %unit m
    ...
    % A = Proj(:,:,100); 
    %% Parameter settings
    nx = 400;ny = nx; % DPCT image size
    dx = single(40E-6);dy= dx;  %pixel size: dx*dy; Default Unit: m dy_det/dx=nx/px
    dy_det=single(40E-6); %detector pixel size, Default Unit: m
